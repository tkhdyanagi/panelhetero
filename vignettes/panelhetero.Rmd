...
vignette: >
  %\VignetteIndexEntry{**panelhetero**}
...
This vignette discusses how to use the **panelhetero** package.

The package enables to estimate the degree of heterogeneity across 
cross-sectional units in the panel data analysis.
...
Suppose that we have balanced panel data $\{ y_{it} \}$ for $i = 1, \dots, N$ and $t = 1, \dots, T$.
Here:
...
the cumulative distribution function (CDF), the density function, and the moments 
(i.e., the means, the variances, and the correlations) of the 
heterogeneous mean, the $k$-th autocovariance, and the $k$-th autocorrelation: 

- $\mu_i = E(y_{it}|i)$,
- $\gamma_{k,i} = E[(y_{it} - \mu_i)(y_{i,t-k} - \mu_i)|i]$,
- $\rho_{k,i} = \gamma_{k,i}/\gamma_{0,i}$,

where $E(\cdot | i)$ denotes the population mean for the individual time series.
...
Note that $\gamma_{0, i}$ corresponds to the variance.
These variables are treated as random variables that are i.i.d. across units.
...
based on the estimated $\hat \mu_i$, $\hat \gamma_{k,i}$, and $\hat \rho_{k,i}$.
...
based on the estimated $\hat \mu_i$, $\hat \gamma_{k,i}$, and $\hat \rho_{k,i}$.
Since these estimators have asymptotic biases, the papers propose 
the half-panel jaccknife (HPJ) bias correction and 
the third-order jackknife (TOJ) bias correction.
In addition, the papers also consider the cross-sectional boostrap procedures 
for performing statistical inference.
The HPJ bias correction requires that $T \ge 4$.
The TOJ bias correction requires that $T \ge 6$.

# Functions

The **panelhetero** package provides the following functions:

- `nemoment()`: The naive estimation of the moments of 
the heterogeneous mean, the heterogeneous autocovariance, and 
the heterogeneous autocorrelation without bias correction 
- `hpjmoment()`: The HPJ bias-corrected estimation of the moments of 
the heterogeneous mean, the heterogeneous autocovariance, and 
the heterogeneous autocorrelation
- `tojmoment()`: The TOJ bias-corrected estimation of the moments of 
the heterogeneous mean, the heterogeneous autocovariance, and 
the heterogeneous autocorrelation
...
The parameters of interest are the means, the variances, and the correlations:
...
The parameters of interest are the CDF of $\mu_i$, that of $\gamma_{k,i}$, 
and that of $\rho_{k,i}$.
...
Then, using an internal function `panelhetero:::simulation()`,
